...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    if (cell == nil)
    {
        cell = [[foodViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"cell"];
    }
    return cell;
}
...
-(void)setFd:(food *)fd{
    ...
    _fd = fd;
    NSURL * url = [NSURL URLWithString:fd.cover_route_map_cover];
    
    
    
    [self.foodImageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"placeholder.jpeg"]];
    
    self.foodlNameView.text = fd.name;
    self.foodDateView.text = fd.date_added;
