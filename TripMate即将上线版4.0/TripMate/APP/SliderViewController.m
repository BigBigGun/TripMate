...
#define OFFSET_X (SCREEN_WIDTH - 60)
#define FINAL_MAINVIEW_HEGIHT 300
...
- (UIImageView *)bgImageView
{
    if (!_bgImageView) {
        ...
        self.bgImageView.image = [UIImage imageNamed:@"bgimage.jpg"];
...
- (void)rigthPanAction:(UIPanGestureRecognizer *)pan
{
    ...
    UIView *view = pan.view;
    CGPoint point = [pan translationInView:view];
    CGFloat offsetX = point.x; //手拖拽多远就偏移多远
    
    if (pan.state == UIGestureRecognizerStateChanged) {
        if (offsetX - self.startPoint.x >= -20) {
            
              //视图的大小随着偏移量的变化而变化
            self.rootVC.view.frame = CGRectMake(offsetX, offsetX / self.offsetX * ((SCREEN_HEGIHT - self.MainLastHegiht) / 2), SCREEN_HEGIHT, SCREEN_HEGIHT - 2 * (offsetX / self.offsetX * ((SCREEN_HEGIHT - self.MainLastHegiht) / 2)));
...
- (void)setPushLeftViewButtonWithImage:(UIImage *)image
{
    ...
    mainVC.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:@selector(handleLeftButton:)];
}
...
//    [self setPushLeftViewButtonWithString:@"左"];
